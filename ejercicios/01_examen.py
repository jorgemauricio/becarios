...
def main():
    ...
    for i in range(1, 6):
        ...
        dias = data[data['Dia'] == i]
        dias = dias[(dias['Lat'] >= 21.0) & (dias['Lat'] <= 24.0)]
        dias = dias[(dias['Long'] >= -104.0) & (dias['Long'] <= -100.0)]
        dias.to_csv('resultados/filtrado(Dia{}).csv'.format(i))
        dias = dias.mean()
        print('Generando el csv de media del fltrado de informacion por Dia {}'.format(i))
        dias.to_csv('resultados/MediaFiltrado(Dia{}).csv'.format(i))
    print("*"*10)
